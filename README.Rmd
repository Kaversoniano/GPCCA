...
```{r}
par(pty = "s", xpd = TRUE, mar = c(5.1, 4.1, 4.1, 5.1))
par(mfrow = c(1, 2))
plot(x = LFs[, 1], y = LFs[, 2], col = grps,
     type = "p", pch = 16, cex = 0.8, asp = 1,
     xlab = "Latent Factor 1", ylab = "Latent Factor 2",
     main = "Colored by \n identified clusters")
legend("right", inset = c(-0.5, 0), title = "Cluster",
       legend = paste0("cluster ", levels(as.factor(as.numeric(grps)))),
       col = 1:length(grps), pch = 16, cex = 0.8)
plot(x = LFs[, 1], y = LFs[, 2], col = factor(lbls),
     type = "p", pch = 16, cex = 0.8, asp = 1,
     xlab = "Latent Factor 1", ylab = "Latent Factor 2",
     main = "Colored by \n ground truth labels")
legend("right", inset = c(-0.5, 0), title = "Group",
       legend = levels(factor(lbls)),
       col = 1:6, pch = 16, cex = 0.8)
par(mfrow = c(1, 1))
```
