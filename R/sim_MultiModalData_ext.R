...
sim_MultiModalData_ext <- function(missVal = TRUE, propMCAR = 0.2, dataType = NULL) {
  ...
  SDs <- diag(1.5 * rbeta(round(m1 * sgl.p), 1, 1))
  ...
  SDs <- diag(1.5 * rbeta(round(m2 * sgl.p), 1, 1))
  ...
  SDs <- diag(1.5 * rbeta(round(m3 * sgl.p), 1, 1))
  ...
  ### Top-hierarchy sampling
  Y1 <- matrix(NA, nrow = nrow(X1), ncol = ncol(X1))
  
  for (k in 1:nrow(X1)) {
    for (i in 1:ncol(X1)) {
      if (dataType == "count") {
        Y1[k, i] <- rpois(n = 1, lambda = exp(X1[k, i]))
      }
      if (dataType == "binary") {
        Y1[k, i] <- rbinom(n = 1, size = 1, prob = 1 / (1 + exp(-X1[k, i])))
      }
    }
  }
  
  Y2 <- matrix(NA, nrow = nrow(X2), ncol = ncol(X2))
  
  for (k in 1:nrow(X2)) {
    for (i in 1:ncol(X2)) {
      if (dataType == "count") {
        Y2[k, i] <- rpois(n = 1, lambda = exp(X2[k, i]))
      }
      if (dataType == "binary") {
        Y2[k, i] <- rbinom(n = 1, size = 1, prob = 1 / (1 + exp(-X2[k, i])))
      }
    }
  }
  
  Y3 <- matrix(NA, nrow = nrow(X3), ncol = ncol(X3))
  
  for (k in 1:nrow(X3)) {
    for (i in 1:ncol(X3)) {
      if (dataType == "count") {
        Y3[k, i] <- rpois(n = 1, lambda = exp(X3[k, i]))
      }
      if (dataType == "binary") {
        Y3[k, i] <- rbinom(n = 1, size = 1, prob = 1 / (1 + exp(-X3[k, i])))
      }
    }
  }
  ...
  if (!missVal) {
    return(list(M1 = t(Y1), M2 = t(Y2), M3 = t(Y3)))
  }
  ...
  if (missVal) {
    ...
    Y1.ic <- MAR.generator(Y1, propMCAR)
    Y2.ic <- MAR.generator(Y2, propMCAR)
    Y3.ic <- MAR.generator(Y3, propMCAR)
    
    return(list(M1 = t(Y1.ic), M2 = t(Y2.ic), M3 = t(Y3.ic)))
  }
}
