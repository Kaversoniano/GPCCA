#' Fit the GPCCA-Bern Model on a General Multi-modal Dataset of Binary (0/1) Observations
#'
#' Implementation of GPCCA-Bern on a general multi-modal dataset of binary (0/1) observations 
...
#' }
#'
#' @seealso \code{\link{GPCCAmodel2}}, \code{\link{EM_completeDAT_Bern}}
#'
#' @examples
#' \dontrun{
#' 
#' ## Generate a 3-modality binary dataset with 20% MCARs
#' DAT <- example_MultiModalData_sim(dataType = "binary", missVal = TRUE, propMCAR = 0.2)
#' 
#' ## Fit GPCCA-Bern model to the incomplete example multi-modal dataset
#' GPCCA.fit <- EM_missingDAT_Bern(Y.list = DAT, d = 4, lambda = 0.1)
#' 
...
EM_missingDAT_Bern <- function(Y.list, d = 2, lambda = 0.1, tol = 0.0001, maxiter = 50,
  ...
  O <- (!is.na(Y)) * 1 # indicator matrix (1 = observed, 0 = missing)
  
  m <- nrow(Y) # number of total dimensions
  n <- ncol(Y) # sample size
  
  ### Parameter Initialization (recommended)
  mu0 <- logit(apply(Y, 1, mean, na.rm = TRUE)) %>% as.vector() # fixed
  
  if (W.init == "PCA") {
    tempY <- scale(x = t(Y), center = TRUE, scale = FALSE)
    W0 <- pcaMethods::pca(tempY, nPcs = d, method = "ppca")@loadings # PPCA
  }
  ...
  data {
    ...
    int<lower=1> N_obs;
    int<lower=1, upper=m> ind_O_k[N_obs];
    int<lower=0, upper=1> Y_k_obs[N_obs];
  }
  ...
  model {
    ...
    Z_k ~ normal(0, 1);
    Y_k_obs ~ bernoulli_logit(X_k[ind_O_k]);
  }
  "
  ...
  while(RMSE > tol & iter < maxiter) {
    ...
    for(k in 1:n) {
      data_k <- base_data
      data_k$N_obs <- length(ind.O[[k]])
      data_k$ind_O_k <- ind.O[[k]]
      data_k$Y_k_obs = Y[ind.O[[k]], k]
    ...
    ########## M-step ##########
    fit.param <- optim(par = as.vector(cbind(mu, W)), fn = Q_rcpp1, gr = G_rcpp1,
                       Y = Y, Z_samples = Z.samples,
                       lambda = lambda, n = n, m = m, d = d, S = S,
                       ind_M = ind.M, mu_t = mu, W_t = W,
