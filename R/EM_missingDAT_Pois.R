#' Fit the GPCCA-Pois Model on a General Multi-modal Dataset of Counts
#'
...
#' }
#'
#' @seealso \code{\link{GPCCAmodel2}}, \code{\link{EM_completeDAT_Pois}}
#'
#' @examples
#' \dontrun{
#' 
...
EM_missingDAT_Pois <- function(Y.list, d = 2, lambda = 0.1, tol = 0.0001, maxiter = 50,
  ...
  mu0 <- log(apply(Y, 1, mean, na.rm = TRUE)) # fixed
  ...
  model {
    ...
    Z_k ~ normal(0, 1);
    Y_k_obs ~ poisson_log(X_k[ind_O_k]);
  }
  "
  ...
  while(RMSE > tol & iter < maxiter) {
    ...
    for(k in 1:n) {
      ...
      Y.mS <- matrix(Y[, k], nrow = m, ncol = S)
      Y.mS[ind.M[[k]], ] <- exp(mu[ind.M[[k]]] + W[ind.M[[k]], , drop = FALSE] %*% Z_k.samples)
      
      B_k <- rowSums(exp(W %*% Z_k.samples))
      A_k <- rowSums(Y.mS)
    ...
    fit.W <- optim(par = as.vector(W), fn = Q_W_rcpp1, gr = G_W_rcpp1,
                   mu = mu, Y = Y, Z_samples = Z.samples,
                   lambda = lambda, n = n, m = m, d = d, S = S,
                   ind_M = ind.M, W_t = W,
                   method = "L-BFGS-B", control = list(trace = 0, maxit = 100))
    W <- matrix(fit.W$par, nrow = m, ncol = d)
