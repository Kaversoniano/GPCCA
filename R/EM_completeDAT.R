#' Fit the GPCCA Model on a Complete Multi-modal Dataset
#'
#' Implementation of Generalized Probabilistic Canonical Correlation Analysis (GPCCA) 
#'   on a complete multi-modal dataset without any missing value. 
...
#' }
#'
#' @seealso \code{\link{GPCCAmodel}}, \code{\link{EM_missingDAT}}, \code{\link{EM_diagonalCOV}}
#'
#' @examples
#' \dontrun{
#' 
#' ## Generate a 3-modality continuous dataset with no missing value
...
#' GPCCA.fit <- EM_completeDAT(X.list = DAT, d = 4, lambda = 0.5)
#'                             
...
EM_completeDAT <- function(X.list, d = 2, lambda = 0.5, tol = 0.0001, maxiter = 100,
  ...
  mu0 <- apply(X, 1, mean) # fixed
  
  if (W.init == "PCA") {
    W0 <- rsvd::rpca(A = t(X), k = d, center = FALSE, scale = FALSE)$rotation # RPCA
  }
  if (W.init == "rand.std.norm") {
    W0 <- matrix(rnorm(n = nrow(X) * d, 0, 1), nrow = nrow(X), ncol = d) # random
  }
  
  Psi0 <- Matrix::bdiag(lapply(lapply(X.list, t), cov, method = "pearson")) # fixed
  ...
  while(RMSE > tol & iter < maxiter) {
    ...
    if (blockwise_inv == 0) {
      inv.Psi <- Matrix::solve(Psi, tol = 0)
    }
    
    if (blockwise_inv == 1) {
      inv.Psi <- Matrix::bdiag(lapply(ms.index, function(mj.index) Matrix::solve(Psi[mj.index, mj.index], tol = 0)))
    }
    
    ########## E-step ##########
    M <- Matrix::solve(diag(d) + t(W) %*% inv.Psi %*% W, tol = 0)
    
    E.z <- M %*% t(W) %*% inv.Psi %*% (X - mu)
    sum_E.z <- rowSums(E.z)
    ...
    fCore.rs <- fCore_missingF(as.matrix(E.z), as.matrix(E.x), as.matrix(M), n, m, d)
  ...
  ### Values
  return(list(Z = as.matrix(Z.new), mu = mu, W = W, Psi = Psi))
}
