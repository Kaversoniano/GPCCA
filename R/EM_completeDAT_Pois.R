#' Fit the GPCCA-Pois Model on a Complete Multi-modal Dataset of Counts
#'
#' Implementation of GPCCA-Pois on a complete multi-modal dataset of counts 
...
#' }
#'
#' @seealso \code{\link{GPCCAmodel2}}, \code{\link{EM_missingDAT_Pois}}
#'
#' @examples
#' \dontrun{
#' 
#' ## Generate a 3-modality count dataset with no missing value
#' DAT <- example_MultiModalData_sim(dataType = "count", missVal = FALSE)
#' 
#' ## Fit GPCCA-Pois model to the complete example multi-modal dataset
#' GPCCA.fit <- EM_completeDAT_Pois(Y.list = DAT, d = 4, lambda = 0.1)
#' 
...
EM_completeDAT_Pois <- function(Y.list, d = 2, lambda = 0.1, tol = 0.0001, maxiter = 50,
  ...
  while(RMSE > tol & iter < maxiter) {
    ...
    fit.W <- optim(par = as.vector(W), fn = Q_W_rcpp0, gr = G_W_rcpp0,
