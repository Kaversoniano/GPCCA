...
#'   no missing value, setting \code{missVal = FALSE} can reduce running time
...
#' ## (`GPCCAmodel` can handle both complete and incomplete data)
#' GPCCA.fit <- GPCCAmodel(X.list = example_MultiModalData_list0,
#'                         d = 4, lambda = 0.5)
#' 
#' ## III: Fit GPCCA model to the complete example multi-modal dataset
#' ## (setting `missVal = FALSE` explicitly to reduce running time)
#' GPCCA.fit <- GPCCAmodel(X.list = example_MultiModalData_list0,
#'                         d = 4, lambda = 0.5, missVal = FALSE)
#' 
#' ## Extract the fitted latent factors
#' LFs <- t(GPCCA.fit$Z)
#' 
#' ## Data visualization
#' lbls <- example_MultiModalData_labels (ground truth sample labels)
#' 
...
GPCCAmodel <- function(X.list, d = 2, lambda = 0.5, tol = 0.0001, maxiter = 100,
  ...
  ### Argument: X.list
  stopifnot(!is.na(X.list), is.list(X.list))
  if (!all(sapply(X = X.list, FUN = is.matrix))) {
  ...
  if (missVal == FALSE) {
    if (sum(na.flags) > 0) {
      warning("If the input multi-modal data 'X.list' contain any missing value as NA, ",
              "the default setting 'missVal = TRUE' should be used.")
      stop("'missVal = FALSE' is specified explicitly but the input of 'X.list' contains missing value.")
    }
  }
